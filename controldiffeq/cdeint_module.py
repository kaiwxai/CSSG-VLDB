...
class VectorFieldGDE_dev(torch.nn.Module):
    def __init__(self, dX_dt, func_f, func_g, rw_adj_in, rw_adj_out):
        """Defines a controlled vector field.
        ...
        self.func_f = func_f
        self.func_g = func_g
        self.rw_adj_in = rw_adj_in
        self.rw_adj_out = rw_adj_out
    ...
    def __call__(self, t, hz):
        ...
        vector_field_g = self.func_g(z, self.rw_adj_in, self.rw_adj_out) # vector_field_g: torch.Size([64, 207, 32, 2])
...
def cdeint_gde_dev(dX_dt, h0, z0, func_f, func_g, t, rw_adj_in, rw_adj_out, adjoint=True, **kwargs):
    r"""Solves a system of controlled differential equations.
    ...
    odeint = torchdiffeq.odeint_adjoint if adjoint else torchdiffeq.odeint
    vector_field = VectorFieldGDE_dev(dX_dt=dX_dt, func_f=func_f, func_g=func_g, rw_adj_in=rw_adj_in, rw_adj_out=rw_adj_out)
    init0 = (h0,z0)
